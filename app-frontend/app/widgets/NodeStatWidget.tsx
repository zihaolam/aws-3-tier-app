...
const NodeDetails: FC<{ nodeStat: NodeStat }> = ({ nodeStat }) => (
	<div className="flex flex-col px-2 py-1 bg-gray-100 border border-gray-200 border-opacity-30 rounded w-64 relative">
		...
		<span className="text-xs grid grid-cols-4 ml-1">
			...
			<span className={`font-medium ${nodeStat.utilization.cpu < 80 ? "text-green-600" : "text-red-600"}`}>{nodeStat.utilization.cpu}%</span>
		</span>
		<span className="text-xs grid grid-cols-4 ml-1">
			<span className="col-span-3">- Memory Utilization: </span>
			<span className={`font-medium ${nodeStat.utilization.memory < 80 ? "text-green-600" : "text-red-600"}`}>
