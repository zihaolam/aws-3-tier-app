...
const NodeStatChart: FC<{ nodeStat: NodeStat }> = ({ nodeStat }) => {
    ...
    const cpuUtilizationOptions: Highcharts.Options = useMemo(() => {
        ...
        cpuUtilizationLineDataStore.current.push([
            dateNow.toLocaleTimeString(),
            nodeStat.utilization.cpu,
        ]);

        if (cpuUtilizationLineDataStore.current.length > 10)
            cpuUtilizationLineDataStore.current =
                cpuUtilizationLineDataStore.current.slice(-10);
        ...
        return {
            ...
            series: [
                {
                    type: "line",
                    data: cpuUtilizationLineDataStore.current,
                    color: "#EC7211",
                },
            ],
        };
...
const BenchmarkPrompt = () => {
    ...
    const { isFetching } = useExecuteBenchmarkPosts({
        ...
        onSuccess: (data) => {
            ...
            setCumulativeStats((prev) => ({
                totalTimeTaken: data.timeTakenInMS + prev.totalTimeTaken,
                totalApiCalls: prev.totalApiCalls + 1,
            }));
            if (!promptRef.current) return;
            promptRef.current.scroll({
    ...
    return (
        <>
            <div className="flex mx-8 items-center gap-x-1 mb-2">
                ...
                {cumulativeStats.totalApiCalls !== 0 && !isQueryingEnabled && (
                    <button
                        ...
                        onClick={() =>
                            setCumulativeStats(defaultCumulativeStats)
                        }
                    >
                        Reset
                    </button>
            ...
            <div className="border border-gray-150 mx-8 rounded mb-5">
                <div
                    className="h-72 max-h-72 overflow-y-scroll border-b border-gray-200"
                    ref={promptRef}
                >
                    {executionPrompt.map((executionResult) => (
                        <div
                            ...
                            key={
                                executionResult.timeStarted +
                                executionResult.timeEnded
                            }
                        >
                            Read executions completed in{" "}
                            <span className="text-green-500">
                                {executionResult.timeTakenInMS}ms
                            </span>
                            , backend served by:{" "}
                            <span className="text-purple-500">
                                {ipMapping[executionResult.apiServedBy]}
                            </span>
                            , db served by:{" "}
                            <span className="text-pink-500">
                                {ipMapping[executionResult.dbServedBy]}
                            </span>
                        </div>
                    ))}
                </div>
                <div className="py-1 px-2 text-sm bg-gray-100">
                    Total API Calls: {cumulativeStats.totalApiCalls}, Total Time
                    Taken: {cumulativeStats.totalTimeTaken}ms, Average Query
                    Time:{" "}
                    {cumulativeStats.totalApiCalls !== 0 &&
                    cumulativeStats.totalTimeTaken !== 0
                        ? (
                              cumulativeStats.totalTimeTaken /
        ...
        <>
            ...
            <div className="flex flex-wrap gap-2 mx-8 mb-8">
                {groupedNodeStat
                    .filter(
                        ([nodeGroup, nodeStats]) =>
                            nodeGroup === "db" || nodeGroup === "app"
                    )
                    .map(([nodeGroup, nodeStats]) =>
                        nodeStats.map((nodeStat) => (
                            <NodeStatChart
                                nodeStat={nodeStat}
                                key={nodeStat.node_name}
                            />
                        ))
                    )}
            </div>
