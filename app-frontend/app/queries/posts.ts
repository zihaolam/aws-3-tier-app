...
const useQueryPosts = (options?: UseQueryOptions<Post[], unknown, Post[], string[]>) =>
	useQuery(["posts"], () => axios.get<Post[]>("http://15.220.241.25/post/").then((res) => res.data), options);

const generateSpamPostsPromises = (numberOfExecutions: number) =>
	new Array(numberOfExecutions).fill(0).map(() => axios.get<Post[]>("http://15.220.241.25/post/").then((res) => res.data));
